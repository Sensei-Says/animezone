...
export class Kodik extends BaseVideoExtractor {
  ...
  private async fetchGet(
    ...
    const proxyUrl = url.replace('https://kodik.info', '/api/kodik');
    console.log('Using proxy URL:', proxyUrl, 'instead of:', url);
  ...
  private async fetchPost(
    ...
    // Заменяем URL на прокси для обхода CORS
    const proxyUrl = url.replace('https://kodik.info', '/api/kodik');
    console.log('Using proxy URL for POST:', proxyUrl, 'instead of:', url);
